...
function [coord,centelem,elem,esurn1,esurn2,nsurn1,nsurn2,bedge,inedge,...
    ...
    satlimit,pormap,bcflag,courant,totaltime,keyfile,keypath1,keypath2,...
    foldername,kmap,wells,porousarea,ds,tconv,fract,elem_post,regionfract,...
    coord_post,formethod,timedimkey,phflw,knownb,rowposit,region,numsline,...
    substeps,maxsteps,benchmark,jointnodes] = preprocessor
...
if strcmp(keyopen,'y')
    command = sprintf('gmsh %s\\%s\\%s',char(keypath2),foldername,char(mshfile));
...
%Guard "fractureoptions.opt" in the new folder created (this is done when a
%folder is generated

%Building the "command" to be used in the dos's prompt
command = sprintf('copy fractureoptions.opt %s\\%s',char(keypath2),foldername);
%Transfer "Start.dat" using the dos's command "copy"
system(command);
...
%             inode = 1;
                ...
                %matrix 
%                 while inode < 6 && elem(ielem,inode) > 0 
...
%                         inode = inode + 1;
%                     elseif elem(ielem,5) ~= 0 && elem(ielem,inode) ~= i
%                         %A special case where the first node evaluated into 
%                         %row "ielem" is different of value "i" but does not 
%                         %is neighbour of node "i". This happen only when
%                         %the node evaluated is the first and the node who
...
if strcmp(keyvisc,'y')
    ...
    %"visc" reads the file puted inside work folder
...
% %-------------------------------------------------------------------------%
% % Ajuste da estrutura de dados -------------------------------------------%
% %-------------------------------------------------------------------------%     
% numfrat=sum(elem(:,5)>2000);
% sk=size(kmap,1);
% af=zeros(1,1);
% %---------------------- Amount of fractures types ------------------------%
% am=max(elem(:,5));
% t=1;
% for i=1:am
%     if i<=am-size(afrat,1)
%        af(i)=0; 
%     else
%        af(i)=afrat(t);
%        t=t+1;
...
% end
% %-------------------------------------------------------------------------%
% %------------- Ajuste dos pontos de intersecção das fraturas -------------%
%     pontfrat=sum(elem(:,2)==0);
%     elem(1:numfrat,5)=elem(1:numfrat,5)-2000+sk;
%     elem(size(elem,1)-pontfrat+1:size(elem,1),5)=...
%     elem(size(elem,1)-pontfrat+1:size(elem,1),5)-2000+sk;
%-------------------------------------------------------------------------%
%-------------------------------------------------------------------------%
for j=1:size(elem,1)
...
function [head,keyfile,mshfile,keypath1,keypath2,basename,complemfile,...
    ...
    bcflag,numbcnode,bcnodepointer,courant,timedimkey,totaltime,numwell,...
    well] = getdatafile
...
%--------------------------------------------------------------------------
% function [bedge,inedge,knownb] = getinfoedge(coord,elem,nnode,nbe,nelem,...
%           nodelim,nodefrat,intnode,flaglim,filepath,bcflag)
...
%         knownb = union(knownb,bedgnumrow(boundpoint));
%     end  %End of FOR
%     knownb = knownb(2:length(knownb));
% end  %End of IF
% 
...
%     amountedges = sum(elem(ielem,1:5) ~= 0);        

...
for index = 1:npar
    [ii,jj] = size(intinterface);
    for aa = 1: jj
        
        if ~isempty(intinterface{aa,index})
            coarseface = intinterface{aa,index};
            refelm = elemloc(inedge(coarseface,3)) == index;
            vec = (-1)*ones(size(refelm,1),1);
            vec(refelm) = 0;
            coarseConsv1(index) = coarseConsv1(index) + sum(flowMs(coarseface).*vec);
        end
    end    
end
for index = 1:npar

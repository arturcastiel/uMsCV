...
if numwell > 0 && any(well(:,9) == 1)
    ...
    for istrwell = 1:length(numstrwell)
        ...
        %%ALTERACOES DE POCO PAR RODAR SPE
        ...
        %%CHECAGEM APROXIMADA
        % ADD ON HERE - mexeu poco
        SPEflag = 0;

            
        if SPEflag == 1
         if sum(fstnodex) == 0 || sum(fstnodey) == 0
            tolx = 0.55;
            toly = 0.55;
            fstnodex = logical( abs( coord(:,1) - well(numstrwell(istrwell),2)) < tolx  );
            fstnodey = logical(abs(coord(:,2) - well(numstrwell(istrwell),2)) < toly);
         end
        end
        ...
        %leastDist(wellnode,  centelem(:,1:2), coord, well(istrwell,4))
       
        poselem = esurn2(wellnode) + 1:esurn2(wellnode + 1);
        %SPE ALTERACAO PRA VER SE RODA ESTA BOSTA TODA BUGADA
        if SPEflag == 1 
            elecad = leastDist(wellnode,  centelem(:,1:2), coord, 30);
        end
        ...
        j = j + length(poselem);
        elseif SPEflag == 1;
             wells(j + 1:j + length(elecad),1) = elecad;
             wells(j + 1:j + length(elecad),2) = countwell;
             wells(j + 1:j + length(elecad),3) = well(numstrwell(istrwell),5);
             wells(j + 1:j + length(elecad),4) = well(numstrwell(istrwell),6);
             wells(j + 1:j + length(elecad),5) = well(numstrwell(istrwell),7);
             wells(j + 1:j + length(elecad),6) = well(numstrwell(istrwell),8);
            j = j + length(elecad);
        end

...
%plot(points(:,1), points(:,2), 'LineWidth',2, 'color', [0 0 0],'LineStyle','--')
    ...
    colormat = load('color3.dat');
    %colormat = colormap(lines);
    %nn = max(elemloc1);
    %nb = size(colormat,1);
% 
%     for el = find(~bad)'
...
%             nloc = elemloc(el);
%             y = round(nloc *nb/nn);
%             fill(elpoint(:,1),elpoint(:,2),colormat(nloc,:),'LineStyle','none')
%             hold on
%         end
%     end
...
ref = ~all(aux,2);
%ref(raux) = false;
ggray = [0.85 0.85 0.85]
wwhite = [0 0 0]

for ii = find(ref)'
    nodes = inedge(ii,1:2)';
    el_face = coord(nodes,:);
    plot(el_face(:,1), el_face(:,2), 'LineWidth',0.5, 'color', ggray ,'LineStyle','-')
end
...
%refb = xor(ismember(elemloc(inedge(edgesOnCoarseBoundary,3:4)),lb),2);
aa = ismember(elemloc(inedge(edgesOnCoarseBoundary,3:4)),lb);
refb = xor(aa(:,1), aa(:,2));

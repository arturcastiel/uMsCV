classdef meshQuad
    ...
    methods (Static)
        ...
        function [x] = JacobiGL(alpha,beta,N)
            ...
            [xint,~] = meshQuad.JacobiGQ(alpha+1,beta+1,N-2);
            x = [-1, xint', 1]';
        end
        ...
        function dP = dJacobiP(r, alpha, beta, N)
            ...
            if(N == 0)
                ...
                dP = sqrt(N*(N+alpha+beta+1))*meshQuad.JacobiP(r(:),alpha+1,beta+1, N-1);
            end;
        end
        ...
        function [V1D] = Vandermonde1D(N,r)
            ...
            for j=1:N+1
                V1D(:,j) = meshQuad.JacobiP(r(:), 0, 0, j-1);
            end;
        end
        ...
        function plotVertices(EtoV,VX,VY)
            ...
            % Plot Nodes number
            for i = 1:nVert
                text(VX(i),VY(i),int2str(i),'fontsize',7,....
                    'fontweight','bold','Color','k');
            end
            % Plot Element number
            for e = 1:nElem
                pos = [mean(VX(EtoV(e,:))),mean(VY(EtoV(e,:)))];
                text(pos(1),pos(2),int2str(e),'fontsize',7,...
    ...
    methods
        function obj = meshQuad(Pdeg,elem,coord) % The Constuctor
            ...
            [KXI,WXI] = meshQuad.GaussLobatto(Pdeg+1);
